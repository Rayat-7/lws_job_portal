...
const getJobs = async (req, res) => {
    try {
        const { page = 1, limit = 10, search, type, experienceLevel, skills, salaryRange, minSalary, maxSalary, sort } = req.query;
        ...
        // Salary Range
        const buildSalaryCondition = (range) => {
            switch (range) {
                case '$0 - $50k':
                    return { salaryMin: { [Op.lte]: 50000 } };
                case '$50k - $100k':
                    return {
                        [Op.and]: [
                            { salaryMin: { [Op.lte]: 100000 } },
                            { salaryMax: { [Op.gte]: 50000 } }
                        ]
                    };
                case '$100k - $150k':
                    return {
                        [Op.and]: [
                            { salaryMin: { [Op.lte]: 150000 } },
                            { salaryMax: { [Op.gte]: 100000 } }
                        ]
                    };
                case '$150k+':
                    return { salaryMax: { [Op.gte]: 150000 } };
                default:
                    return null;
            }
        };
        ...
        if (salaryRange) {
            const selectedRanges = salaryRange.split(',').map(range => range.trim()).filter(Boolean);
            const salaryConditions = selectedRanges.map(buildSalaryCondition).filter(Boolean);

            if (salaryConditions.length > 0) {
                where[Op.and] = where[Op.and] || [];
                where[Op.and].push({ [Op.or]: salaryConditions });
            }
        } else {
            if (minSalary) {
